...
def main():
    ...
    x = [[model.NewIntVar(0, 1, f'next x {i} {j}') for i in range(data['NumNodes']+1)] for j in range(data['NumNodes']+1)]
    y = [[model.NewIntVar(0, 1, f'next y {i} {j}') for i in range(data['NumNodes']+1)] for j in range(data['NumNodes']+1)]
